...
class CustomModel(nn.Module):
    """
    自定义模型，集成 ResFBlock 模块，移除了 ResNeXtBlock
    """

    def __init__(self, input_dim=6000, num_classes=10, embed_dim=32, window_size=4, num_heads=2,
                 focal_level=3, focal_x=[3, 1, 1, 1], focal_factor=2, mlp_ratio=4., drop=0.3, drop_path=0.):
        ...
        # LSTM
        self.lstm = nn.LSTM(input_size=embed_dim, hidden_size=64, batch_first=True, bidirectional=False)
...
class PRDataset(Dataset):
    def __init__(self, csv_file, train=True, lens=640, num_classes=10, window_size=100):
        """
        Args:
    ...
    def get_peaks(self, signal, threshold):
        """
        识别信号中的局部峰值点。
        """
        peaks = []
    ...
    def generate_pr_plot(self, signal, peaks):
        """
        生成 PR 图。
        """
        pr_plot = np.zeros_like(signal)
        for peak in peaks:
            start = max(peak - self.window_size, 0)
            end = min(peak + self.window_size, len(signal))
            pr_plot[start:end] += signal[start:end]
        return pr_plot

    def extract_features(self, pr_plot):
        """
        从 PR 图中提取统计特征。
        """
        features = []
        features.append(np.mean(pr_plot))
        features.append(np.std(pr_plot))
        features.append(np.max(pr_plot))
        features.append(np.min(pr_plot))
        features.append(np.sum(pr_plot))
        return np.array(features, dtype=np.float32)

    def __getitem__(self, idx):
        row = self.data[idx]
        # 假设第一列是样本ID，最后一列是标签
        features = row[1:-1].astype(float)
        label = int(row[-1])

        # 生成 PR 图
        peaks = self.get_peaks(features, threshold=np.mean(features))
        pr_plot = self.generate_pr_plot(features, peaks)

        # 提取 PR 图特征
        pr_features = self.extract_features(pr_plot)

        # 转换为 tensor
        pr_features = torch.from_numpy(pr_features)
        label = torch.tensor(label, dtype=torch.long)

        return pr_features, label


def get_dataloaders(train_csv, test_csv, batch_size=32, lens=640, num_classes=10, window_size=100):
    train_dataset = PRDataset(csv_file=train_csv, train=True, lens=lens, num_classes=num_classes,
                              window_size=window_size)
    val_dataset = PRDataset(csv_file=test_csv, train=False, lens=lens, num_classes=num_classes, window_size=window_size)
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            for inputs, labels in dataloader:
                inputs = inputs.to(device)  # (batch, 5)
...
def main():
    ...
    # 预测
    test_dataset = PRDataset(csv_file=test_csv, train=False, lens=lens, num_classes=num_classes, window_size=100)
